    ...
    public BinSrchTree<T> subSet(T fromItem, boolean fromInclusive, T toItem, boolean toInclusive) {
        ...
        if (fromInclusive && toInclusive) {
            while (!tree.isEmpty()) {
                ...
                if (item.compareTo(fromItem) == 0 || item.compareTo(toItem) == 0
                        || ((item.compareTo(fromItem) == 1) && (item.compareTo(toItem) == -1))) {
                    sub.Insert(item);
                }
            }
